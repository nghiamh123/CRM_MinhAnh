...
import { rentalService, customerService, uploadService } from "../services/api";
...
const Rentals = () => {
  const { rentals, customers, devices, refreshData, loading } =
    useDataContext();
  ...
  const handleOpenForm = (rental = null) => {
    ...
    if (rental) {
      const cId =
        rental.customerId?.id || rental.customerId?._id || rental.customerId;
      const cust = customers.find((c) => c.id === cId || c._id === cId);

      setFormData({
        customerId: cId,
        devices: rental.devices
          ? JSON.parse(JSON.stringify(rental.devices))
          : [],
        rentalDate: rental.rentalDate
          ? new Date(rental.rentalDate).toISOString().split("T")[0]
          : new Date().toISOString().split("T")[0],
        plannedReturnDate: rental.plannedReturnDate
          ? new Date(rental.plannedReturnDate).toISOString().split("T")[0]
          : "",
      ...
      if (cust) {
        setCustomerFormData({
          ...
          idCardFront: cust.idCardFront || "",
          idCardBack: cust.idCardBack || "",
          idCardSelfie: cust.idCardSelfie || "",
          email: cust.email || "",
          notes: cust.notes || "",
  ...
  const [uploadStatus, setUploadStatus] = useState({
    idCardFront: "idle", // 'idle', 'uploading', 'success', 'error'
    idCardBack: "idle",
    idCardSelfie: "idle",
  });
      ...
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === "rentalDate") {
          ...
          const getName = (c) =>
            c && c.name
              ? c.name
              : customers.find((x) => x.id === c || x._id === c)?.name ||
                "Khách lạ";
  ...
  const handleSubmit = async (e) => {
    ...
    for (const item of formData.devices) {
      ...
      if (
        ...
        const availableUnits = (item.device?.units || []).filter(
          (u) => u.status === "available",
        ).length;
        if (availableUnits >= item.quantity) {
          alert(
            `Bạn chưa chọn đủ số seri cho thiết bị: ${item.device?.name || "Không rõ"}`,
          );
          return;
        }
      }
  ...
  if (loading && rentals.length === 0)
    return <div className="container">Đang tải danh sách đơn thuê...</div>;
    ...
    if (currentSerials.length > newQ) {
      ...
      const availableUnits = (dObj.units || [])
        .filter(
          (u) =>
            u.status === "available" &&
            !currentSerials.includes(u.serialNumber),
        )
  ...
  const availableDevices = devices.filter((d) => {
    const isAvailable =
      d.status === "available" ||
      (editingRental && d.id === editingRental.deviceId);
    const matchesFilter =
      deviceFilterType === "all" || d.type === deviceFilterType;
      ...
      <div className="container">
        <div className="breadcrumb-container">
          ...
          <span className="breadcrumb-item active">
            {view === "add" ? "Tạo đơn mới" : "Cập nhật đơn"}
          </span>
        </div>
        ...
        <div className="form-section-card">
          ...
          <form onSubmit={handleSubmit}>
            <div className="rental-grid-main">
              <div>
                <div className="form-title">
                  <label className="rental-section-title">
                    1. Khách hàng (Chọn có sẵn)
                  </label>
                  <SearchableSelect
                    options={customers}
                ...
                <div className="customer-detail-box">
                  ...
                  <div className="upload-grid">
                    ...
                    <div className="upload-box">
                      <div className="file-input-wrapper">
                        {customerFormData.idCardFront ? (
                          <div className="image-preview-container">
                            <img
                              src={customerFormData.idCardFront}
                    ...
                    <div className="upload-box">
                      <div className="file-input-wrapper">
                        {customerFormData.idCardBack ? (
                          <div className="image-preview-container">
                            <img
                              src={customerFormData.idCardBack}
                              className="image-preview"
                              alt="Back"
                            />
                            <div
                    ...
                    <div className="upload-box">
                      <div className="file-input-wrapper">
                        {customerFormData.idCardSelfie ? (
                          <div className="image-preview-container">
                            <img
                              src={customerFormData.idCardSelfie}
                              className="image-preview"
                              alt="Selfie"
                            />
                            <div
                          ...
                          <>
                            ...
                            <input
                              ...
                              accept="image/*"
                              onChange={(e) =>
                                handleFileUpload(e, "idCardSelfie")
                              }
                            />
                          </>
                        )}
                  ...
                  <div className="rental-checkbox-wrapper">
                    ...
                    />
                    <label htmlFor="isRental" className="rental-checkbox-label">
                      Đánh dấu là Rental (Cửa hàng/Đối tác)
                    </label>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          plannedReturnDate: e.target.value,
                        })
                      }
                    />
                  </div>
              ...
              <div className="cart-wrapper">
                    ...
                    availableDevices.map((device) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          const availableUnits = (device.units || [])
                ...
                <div className="cart-items-container">
                  <label className="cart-items-title">
                    Danh sách đã chọn ({formData.devices.length} thiết bị)
                  </label>
                  {formData.devices.length === 0 ? (
                    <div className="cart-items-empty">
                      Giỏ hàng đang trống. Chọn thiết bị bên trên để thêm vào
                      đơn.
                    </div>
                  ) : (
                      ...
                      <div key={idx} className="cart-item">
                        <div className="cart-item-info">
                          ...
                          <div className="cart-item-price">
                            Đơn giá: {formatVND(item.pricePerDay)}/ngày
                          </div>

                          {/* Phân bổ Số Seri */}
                          <div className="cart-item-sn-wrapper">
                            <div className="cart-item-sn-label">
                              Chọn SN ({item.selectedSerials?.length || 0}/
                              {item.quantity}):
                            </div>
                                ...
                                .map((u) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={
                                        isSelected
                                          ? "sn-badge sn-badge-selected"
                                          : "sn-badge sn-badge-unselected"
                                      }
                                      onClick={() => {
                                        const newDevs = [...formData.devices];
                                        let currentSerials =
                                          newDevs[idx].selectedSerials || [];
                                        if (isSelected) {
                                          currentSerials =
                                            currentSerials.filter(
                                              (sn) => sn !== u.serialNumber,
                                            );
                                        } else {
                                          if (
                                            currentSerials.length <
                                            newDevs[idx].quantity
                                          ) {
                                            currentSerials.push(u.serialNumber);
                                          } else {
                                            alert(
                                              `Chỉ được chọn tối đa ${newDevs[idx].quantity} S/N! Tăng số lượng nếu cần.`,
                                            );
                                          }
                                        }
                                        newDevs[idx].selectedSerials =
            ...
            <div className="flex-between form-actions-wrapper">
              ...
              <button type="submit" className="btn-primary btn-confirm">
            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  className="table-header-cursor"
                >
                  Ngày thuê {getSortIcon("rentalDate")}
                </th>
                <th
                  onClick={() => handleSort("plannedReturnDate")}
                  className="table-header-cursor"
                >
                  Hẹn trả {getSortIcon("plannedReturnDate")}
                </th>
                <th
            ...
            <tbody>
              {sortedRentals.map((rental) => (
                <tr key={rental.id}>
                  ...
                  <td>
                    {rental.devices &&
                      rental.devices.map((dItem, i) => {
                        const name = getDeviceName(dItem.device);
                        return (
                          <div key={i} className="rental-cell-meta">
                            <span className="cell-bold">• {name}</span> (x
                            {dItem.quantity})
                            {dItem.selectedSerials &&
                              dItem.selectedSerials.length > 0 && (
                  ...
                  <td>
                    {rental.plannedReturnDate
                      ? new Date(rental.plannedReturnDate).toLocaleDateString(
                          "vi-VN",
                        )
                      : ""}
                  </td>
