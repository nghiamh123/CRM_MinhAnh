...
const Devices = () => {
  ...
  const sortedDevices = useMemo(() => {
    let sortableItems = devices.filter(
      (d) =>
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingDevice) {
      ...
      const rentedCount = existingUnits.filter(
        (u) => u.status === "renting",
      ).length;
      const maintenanceCount = existingUnits.filter(
  ...
  if (loading && devices.length === 0)
    return <div className="container">Đang tải danh sách thiết bị...</div>;
      ...
      <div className="container">
        ...
        <div className="form-section-card">
          ...
          <h2 className="form-title">
            {view === "add"
              ? "Thêm thiết bị mới"
              : `Sửa thiết bị: ${editingDevice.name}`}
          </h2>
              ...
              <div className="input-currency-wrapper">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      pricePerDay: parseDots(e.target.value),
                    })
                  }
                  placeholder="Nhập giá tiền..."
                />
                <span className="input-currency-label">VNĐ</span>
              </div>
            </div>
            <div className="form-group-mt">
            ...
            <div className="sn-container">
              <div className="flex-between sn-header">
                <label className="sn-label">
                  Số seri (S/N) thiết bị - Đăng ký {formData.totalQuantity} máy
                </label>
              </div>
              <div className="sn-grid">
                {Array.from({ length: formData.totalQuantity }).map(
                  (_, idx) => (
                    <input
                      key={idx}
              ...
              <p className="sn-note">
                * Thay đổi số lượng phía trên để thấy danh sách S/N thay đổi
                tương ứng.
              </p>
            </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          >
            ...
            >
              ...
              <input
                type="text"
                placeholder="Tìm theo tên hoặc loại thiết bị..."
                value={searchTerm}
            ...
            <tbody>
              {sortedDevices.map((device) => (
                <tr key={device.id}>
                  ...
                  <td>
                    <span
                      className={`cell-nowrap ${device.availableQuantity > 0 ? getStatusClass("available") : getStatusClass("late")}`}
                    >
                      {device.availableQuantity > 0 ? "Có sẵn " : "Hết hàng "}(
                      {device.availableQuantity || 0}/
                      {device.totalQuantity || 1})
                    </span>
                    {device.units && device.units.length > 0 && (
                      <div className="device-rảnh-text">
                        {
                          device.units.filter((u) => u.status === "available")
                            .length
                        }{" "}
                        rảnh
                      </div>
